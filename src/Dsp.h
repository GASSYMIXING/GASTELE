...
enum Param : unsigned { Bypass, Character, Style, Mix, Output, Realphone, Count, Meter=100 };
inline constexpr std::array<double,Count> defaults {0., .65, 0., 1., .5, 0.};
...
public:
    ...
    void prepare(double sampleRate) {
        ...
        for(auto& eq:realphone)eq.prepare(sr);
        peak=0;
    }
    ...
    void frame(double* values,int channels) {
        ...
        for(int c=0;c<channels;c++) {
            ...
            // Optional serial post-EQ AFTER the complete legacy chain, including
            // MIX and OUTPUT. It is not another parallel style or dry/wet branch.
            const double equalized=realphone[c].tick(values[c]);
            const double amount=current[Realphone]*(1-current[Bypass]);
            if(amount>0.)values[c]+=(equalized-values[c])*amount;
